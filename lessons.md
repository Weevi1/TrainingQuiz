...
- [2026-04-16] Participants on mobile got "session timed out" and had to rejoin. Root cause: all `onSnapshot` listeners had no error callbacks (died silently on connection drops) + no Firestore offline persistence (no cached data during disconnects) + no tab-visibility recovery (missed state changes while backgrounded). → (1) **Always pass `onError` to `onSnapshot`** — a listener without an error callback is a listener that fails silently. (2) **Enable `persistentLocalCache`** on any Firebase web app where users are on mobile — it's the #1 best practice for mobile reliability. (3) **Add `visibilitychange` handlers** for any critical real-time state on mobile — tabs get suspended and listeners may have a reconnection gap. (4) **Don't conflate "listener error" with "document deleted"** — kick detection must distinguish network loss from actual removal.
