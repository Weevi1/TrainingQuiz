...
**Recent (April 2026):**
- **Mobile connection resilience** (April 2026): Tenant reported participants getting "session timed out" on mobile. Root cause: no Firestore offline persistence + all `onSnapshot` listeners lacked error callbacks + no tab-visibility recovery. Fix: enabled `persistentLocalCache` with multi-tab manager in `firebase.ts`, added `onError` callbacks to all 5 subscription methods in `firestore.ts`, added `visibilitychange` safety-net fetches in PlaySession (catches missed status changes when tab was backgrounded), "Reconnecting..." banner via `metadata.fromCache`, hardened kick detection (connection error ≠ kicked), added error handling + visibility refresh to RTDB timer sync. See `docs/CHANGELOG.md` 2026-04-16 entry.
- **Presenter results hardening + quiz-flow recheck** (April 2026): Fixed a TDZ in `PresenterResultsSummary` (white screen on quiz completion), header "AVG SCORE 300%" bug (raw score templated as %), boss-multiplier-aware percentages everywhere (header, leaderboard column, PDF reports), bingo-specific leaderboard columns, podium avatars, competition ranking for ties, leaderboard cap at top 20 + "+ N more" row, `<HeaderStats />` extracted into a memoized child for perf, `<ResultsErrorBoundary>` around results render, ESLint `no-use-before-define` rule, keyboard a11y on click-to-skip, `useCountUp` snap-to-zero fix, `PlaySession` answer-write retry. Reverted an earlier wrong "fix" that swapped Olympic podium order. See `docs/CHANGELOG.md` 2026-04-10 entry for full list.

**Recent (February–March 2026):** Cinematic presenter results, RTDB clock-synced timers, quiz customization (custom feedback / milestones / boss questions), PDF export overhaul, certificate template system (`elegant` + `cpd-professional`), session-level certificate fields, platform-wide animated stickers (DiceBear removed), participant performance optimizations (lazy routes, org cache, polling instead of live-rank listener), Firestore cost audit (limit + pagination, `userOrgIndex`, removed orphaned `answers` subcollection), single-source podium/leaderboard sort. **See `docs/CHANGELOG.md` for full history.**
